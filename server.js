...
var serverIpAddress = process.env.OPENSHIFT_NODEJS_IP || '192.168.1.4'
var socketIoServer = '192.168.1.4';
...
/*
var options = {
    key: express.readFileSync(path.join(__dirname, 'fake-keys/privatekey.pem')),
    cert: express.readFileSync(path.join(__dirname, 'fake-keys/certificate.pem'))
}; */
...
try {
    ...
    if ((config.serverPort || '').toString() !== '1337') {
...
function serverHandler(request, response) {
    try {
        ...
        try {
            stats = express.lstatSync(filename);

            if (filename && filename.search(/new/g) === -1 && stats.isDirectory()) {
                response.writeHead(200, {
                    'Content-Type': 'text/html'
                });
                response.write('<!DOCTYPE html><html><head><meta http-equiv="refresh" content="0;url=/public/new/"></head><body></body></html>');
                response.end();
                return;
            }
        } catch (e) {
        ...
        if (express.statSync(filename).isDirectory()) {
            response.writeHead(404, {
                'Content-Type': 'text/html'
            });

            if (filename.indexOf('/new/MultiRTC/') !== -1) {
                filename = filename.replace('/new/MultiRTC/', '');
                filename += '/new/MultiRTC/index.html';
            } else if (filename.indexOf('/new/') !== -1) {
        ...
        express.readFile(filename, 'utf8', function(err, file) {
            if (err) {
            ...
            try {
                var demos = (express.readdirSync('new') || []);
                ...
                if (demos.length) {
                    ...
                    demos.forEach(function(f) {
                        if (f && f !== 'index.html' && f.indexOf('.html') !== -1) {
                            otherDemos += '<li><a href="/new/' + f + '">' + f + '</a> (<a href="https://github.com/muaz-khan/RTCMultiConnection/tree/master/demos/' + f + '">Source</a>)</li>';
                        }
                    });
            ...
            try {
                var docs = (express.readdirSync('docs') || []);
...
function runServer() {
    app.on('error', function(e) {
        if (e.code == 'EADDRINUSE') {
            ...
            var socketURL = (isUseHTTPs ? 'https' : 'http') + '://' + e.address + ':' + e.serverPort + '/';

            console.log('------------------------------');
            console.log('\x1b[31m%s\x1b[0m ', 'Unable to listen on serverPort: ' + e.serverPort);
    ...
    app = app.listen(serverPort, process.env.IP || '0.0.0.0', function(error) {
        ...
        var domainURL = (isUseHTTPs ? 'https' : 'http') + '://' + addr.address + ':' + addr.serverPort + '/';
